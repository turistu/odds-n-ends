...
ssh server [[6]](#6) does the socket forwardings (as all the other operations
...
permissions and any `SO_PEERCRED` / `SO_PASSCRED` checks performed by the peer.
...
from one terminal:
```
rm -f ~/sock; ssh -L ~/sock:/run/systemd/private -t -p 222 luser@trixie sleep 71d
```
...
You can change the default port in `/etc/default/dropbear`. The last vulnerable version of dropbear was
[88-2](http://ftp.de.debian.org/debian/pool/main/d/dropbear/dropbear-bin_2025.88-2_amd64.deb).

<a id=5>[5]</a> see https://github.com/mkj/dropbear/blob/7b8e47a7/src/svr-agentfwd.c#L155
...
<a id=6>[6]</a> This was fixed in the 2025.89 version of dropbear.
